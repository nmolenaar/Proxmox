...
default_setup() {
  ...
  #lxc-attach -n "$CTID" -- bash -c "source <(curl -s https://raw.githubusercontent.com/nmolenaar/Proxmox/remz/misc/install.func) && color && verb_ip6 && catch_errors && setting_up_container && network_check && update_os" || exit
  pct exec "$CTID" -- /bin/bash -c "source <(wget -qLO - https://raw.githubusercontent.com/nmolenaar/Proxmox/remz/misc/install.func) && color && verb_ip6 && catch_errors && setting_up_container && network_check && update_os" || exit
